  ...
  return {
    init: function () {
      ...
      $("#editScheduleBtnModal").on("click", function () {
        ...
        $.ajax({
          url: "http://10.104.10.243:8088/api/dutySchedule/updateEmployeeAppraisal",
          type: "POST",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
...
let LoadDutyScheduleRequest = function () {
  ...
  $.ajax({
    url: "http://10.104.10.243:8088/api/dutySchedule/searchDutyScheduleForEmployeeAppraisalForm",
    type: "POST",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
    },
