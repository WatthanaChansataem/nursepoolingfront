    ...
    table = $("#dataTable").DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          title: "แก้ไข",
          render: function (data, type, full, meta) {
            return full.status == dutyScheduleSStatusConstant.Normal
  ...
  return {
    init: function () {
      ...
      $("#editScheduleBtnModal").on("click", function () {
        ...
        $.ajax({
          url: "http://10.104.10.243:8088/api/dutySchedule/update",
          type: "POST",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
...
let LoadDutySchedule = function () {
  ...
  $.ajax({
    url: "http://10.104.10.243:8088/api/dutySchedule/searchDutyScheduleForDutyScheduleFormTable",
    type: "POST",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
    },
