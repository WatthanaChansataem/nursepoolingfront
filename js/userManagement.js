  ...
  return {
    init: function () {
      ...
      containerTable.on("click", ".edit-button", function () {
        ...
        $("#profileImg").attr(
          "src",
          `http://10.104.10.243:8088/api/document/avatar/${data.userId}`
        );
      ...
      $("#editScheduleBtnModal").on("click", function () {
        ...
        $.ajax({
          url: "http://10.104.10.243:8088/api/User/userManagement",
          type: "POST",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
...
let LoadDutySchedule = function () {
  ...
  $.ajax({
    url: "http://10.104.10.243:8088/api/User/searchForUserManagement",
    type: "POST",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
    },
...
$("#addUserBtnModal").on("click", function () {
  ...
  $.ajax({
    url: "http://10.104.10.243:8088/api/User/createAdmin",
    type: "POST",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("token"),
    },
